...
`renderTipTapRichText` ships default mappings for the standard nodes and marks (`textBlock` → `<p>`, `bulletList` → `<ul>`, `orderedList` → `<ol>`, `listItem` → `<li>`, `bold` → `<strong>`, `italic` → `<em>`, `strike` → `<s>`, `superscript` → `<sup>`, `subscript` → `<sub>`, and the special-character nodes). Your `nodeMapping`/`markMapping` are merged over the defaults; supply handlers for anything the defaults don't cover (`link`, `inlineStyle`, `cmsBlock`/`cmsInlineBlock`, `placeholder`). A block with headings needs its own `textBlock` handler, since which tag a text block renders as follows from the block's configuration, which the site doesn't have.
