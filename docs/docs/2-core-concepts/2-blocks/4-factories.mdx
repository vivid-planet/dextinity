...
Values that were stored before an option was left out survive reading: the block loads them unchanged, and `migrateBlocks` writes them back as they are.
They don't survive editing, though — saving a block in the Admin rebuilds its data from the input, which no longer carries the unsupported options.
So narrowing `supports` leaves existing content untouched until an editor saves that block, and drops the values from then on.
...
Narrowing `supports` is therefore not reversible for content that is edited in the meantime.
Widening it again only brings back the values of blocks that nobody saved while the option was left out.
...
As long as the API block still supports them, their stored values are kept — the Admin passes them through unchanged — so widening `supports` shows them in the editor again.
Once the API block leaves them out too, they are dropped as described above.
