...
| ----------- | ------------------------------- | ----------------------------------------- |
| Effect      | Hides the field from the editor | Removes the field from the block entirely |
| Block name  | Fixed to `ExternalLink`         | Mandatory, must be your own               |
...
Both fields stay part of the block's data either way, so the API block and the site component are unaffected.
An option that is left out keeps its default — it isn't forced to a different value.
