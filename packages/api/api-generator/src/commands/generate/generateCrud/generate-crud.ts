...
function generateEntityImport(targetMetadata: EntityMetadata<any>, relativeTo: string, sourceMetadata?: EntityMetadata<any>): Imports[0] {
    // Entities created by a factory (e.g. the DAM file entity) report the factory's file as their path, which is
    // inside the cms-api package rather than the application. Reusing the import of the entity that references them
    // resolves them to the module the application actually exports them from.
    const libMatch = targetMetadata.path.match(/(packages\/api|@dextinity)\/cms-api\/lib\/(.*)/);
    if (libMatch) {
        // Entities created by a factory (e.g. the DAM file entity) report the factory's file inside the cms-api
        // package as their path, even though the application declares the concrete class. Following the import of
        // the entity that references them leads to the file the application actually declares them in.
