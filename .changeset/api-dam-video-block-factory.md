...
Values that were stored before an option was left out survive reading, so narrowing `supports` leaves existing content untouched, but they are dropped the next time an editor saves the block.
