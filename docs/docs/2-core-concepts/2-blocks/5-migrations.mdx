...
An application can extend a block it doesn't own by extending the classes it is built from, `blockDataClass` and `blockInputClass`, and add migrations of its own:

```ts title="captioned-you-tube-video.block.ts"
...
export const CaptionedYouTubeVideoBlock = createBlock(
    CaptionedYouTubeVideoBlockData,
    CaptionedYouTubeVideoBlockInput,
    {
