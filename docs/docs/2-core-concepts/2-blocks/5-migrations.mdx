...
The vendor migrations of a block can only come from one library: a block extending a block that has them can't declare its own, and puts its migrations in `migrate`.
