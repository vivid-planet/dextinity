...
describe("createTipTapRichTextBlock — lists", () => {
    ...
    it("leaves out the spacing below a list that is the last block", () => {
        const listBlock = { type: "bulletList", content: [createListItem("One")] };
        const listInTheMiddle = renderWithTheme(<HtmlTipTapRichTextBlock data={createBlockData([listBlock, createParagraph("After")])} />);
        const listAtTheEnd = renderWithTheme(<HtmlTipTapRichTextBlock data={createBlockData([listBlock])} />);

        expect(listInTheMiddle).toContain("richTextBlock__listItem--blockSpacing");
        expect(listAtTheEnd).not.toContain("richTextBlock__listItem--blockSpacing");
    });
