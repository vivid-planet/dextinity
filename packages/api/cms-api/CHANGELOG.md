...
- ab940c1: Improve performance of the `contentScopesCount` field when querying the users list
...
- 578525c: Fix copying the home page creating a second page with the slug `home`
...
- cfe1979: Validate uploaded SVGs with `DOMPurify` (backed by `jsdom`) instead of the custom XML-based check
...
- d8e7261: Add `ids` filter to `damFilesList`
...
- 1e140d6: Return null in `getNodeByPath` when path is `/home` to prevent the home page from being returned for that path (results in 404)
