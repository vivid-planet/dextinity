...
- End the description with the template's task and session lines as plain visible text, not as HTML comments: `Task: …` (only when there is a task) and `Session: https://claude.ai/code/session_…`, pointing at the current session (only when you have that URL). The session line is the entire attribution: don't add a "Generated with Claude Code" footer, a `_Generated by …_` line, a second session URL or a `Co-authored-by` line to the description.
