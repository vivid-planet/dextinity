...
Their stored values are kept: the API keeps them even when its own `supports` leaves them out, so widening `supports` in both shows them in the editor again.
