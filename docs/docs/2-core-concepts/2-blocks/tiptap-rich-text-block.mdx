...
A node written before the `textBlock` attribute existed doesn't carry a name and is resolved by its tag, so content from before this option keeps working without a migration. Only when several text blocks share a tag does the assignment become ambiguous — the first one with a matching tag wins. `buildApplyTextBlocksMigration` writes the resolved name into the content once, so the assignment stops depending on the configuration's order:
...
Place it after any migration that changes a node's tag (for instance one that bumps heading levels), since it resolves the tag a node has at the point it runs.
