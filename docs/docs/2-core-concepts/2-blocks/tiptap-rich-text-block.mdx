...
| `rte.blocktypeMap` (custom block types)         | [`textBlocks`](#text-blocks) + [`styles`](#text-styles)                  |
| `rte.customInlineStyles`                        | [`inlineStyles`](#inline-styles) + the inline style menu                 |
...
In the Admin, the style and placeholder options additionally carry rendering information (`label` and `element`) that is used to preview them in the editor — see [Inline styles](#inline-styles).
...
It uses the block's enabled features and its `maxTextBlocks` option to build the target schema and validates the result. The conversion is **best effort**: if validation fails, it falls back to a stripped-down plain-text document in production (logging a warning) and throws in development so you can catch problems early.
...
export const TipTapRichTextBlock = createTipTapRichTextBlock({
    link: LinkBlock,
    textBlocks: [{ name: "paragraph", tag: "p", styles: [{ name: "copy200" }] }],
    inlineStyles: [{ name: "highlight" }],
    // highlight-start
    migrateFromDraftJs: {
        // Map the Draft.js `blocktypeMap` entry to a text block and its style.
        textBlockMap: {
            "paragraph-small": { textBlock: "paragraph", textStyle: "copy200" },
        },
...
`inlineStyles` adds named styles that the editor applies to a text selection, offered in the toolbar's "More options" menu. They are configured on the API (`name` + optional `appliesTo`) and in the Admin (additionally `label` for the menu entry, `element` for the editor preview and an optional `icon`). The `appliesTo` option limits a style to certain text block types.
...
On the site, the chosen style is available as the `inlineStyle` mark type, which you resolve to your own components in `markMapping`:

```tsx title="TipTapRichTextBlock.tsx (Site)"
const markMapping: Record<string, TipTapMarkHandler> = {
    inlineStyle: ({ mark, children }) =>
        mark.attrs?.type === "highlight" ? <Highlight>{children}</Highlight> : <>{children}</>,
};
```

### Text styles

A text block can offer `styles` the editor applies to it, shown in a second toolbar select next to the text block type select. A style decouples the typography ("small copy") from the semantics ("this is a paragraph"), so an editor can pick a look without changing the tag the content is stored as.

Styles are configured on the API (`name` only) and in the Admin (additionally `label` for the select and an optional `element` that previews the style in the editor). One style set can serve several text blocks: the `element` is handed the tag its text block is stored as, so the same headline style renders an `<h1>` for one text block and an `<h2>` for the next.

```ts title="tip-tap-rich-text.block.ts (API)"
export const TipTapRichTextBlock = createTipTapRichTextBlock({
    textBlocks: [
        { name: "paragraph", tag: "p", styles: [{ name: "copy300" }, { name: "copy200" }] },
        { name: "heading-1", tag: "h1", styles: [{ name: "headline450" }] },
...
const copyStyles: TipTapTextBlockStyle[] = [
    {
        name: "copy300",
        label: <FormattedMessage id="tipTapRichText.copy300" defaultMessage="Copy" />,
        element: (props, Tag) => <Tag className={styles.copy300} {...props} />,
    },
    {
        name: "copy200",
        label: <FormattedMessage id="tipTapRichText.copy200" defaultMessage="Copy Small" />,
        element: (props, Tag) => <Tag className={styles.copy200} {...props} />,
    },
];
...
export const TipTapRichTextBlock = createTipTapRichTextBlock({
    textBlocks: [
        ...
        // A text block can render its own element without offering styles.
        {
            ...
            element: (props, Tag) => <Tag className={styles.display} {...props} />,
        },
    ],
...
The chosen style is stored in the node's `textStyle` attribute, which the site resolves to its own typography:
...
const nodeMapping: Record<string, TipTapNodeHandler> = {
    textBlock: ({ node, children }) => (
        <Typography variant={node.attrs?.textStyle ?? node.attrs?.textBlock}>{children}</Typography>
    ),
};
```

A style the text block it sits on isn't configured for is rejected during validation, so the `styles` have to match between the API and the Admin. Switching to a text block that doesn't offer the applied style drops it.

#### List styles

Lists take their styles through `orderedList`/`unorderedList`, passing `{ styles }` in place of `true`. The style is stored on the **list** node, not on its items, so all items of one list share it:
...
A nested list is a node of its own and therefore carries its own style, independent of the list around it. The style select always addresses the innermost list the cursor is in, and a list item's own text block never carries a `textStyle` — the list owns it, and content that sets one anyway is rejected.

On the site, a list item reads the style from the list it is rendered in, which `renderTipTapRichText` hands to every handler as `parent`:
