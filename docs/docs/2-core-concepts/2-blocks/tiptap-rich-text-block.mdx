...
### Content schema in the block meta

The API describes the content each TipTap rich text field accepts in the block meta (`block-meta.json`). The `TipTapRichTextBlock` field carries a `schema` next to its `childBlocks`, which is meant for consumers that generate or validate content outside the editor, for instance AI agents writing `tipTapContent` for a block.

The `schema` contains:

- `topNode`, `nodes` and `marks` from the ProseMirror schema: each node's content expression, group, `inline`/`atom` flags and attributes with their defaults, each mark's attributes.
- The rules the API validates on top of the schema, which the ProseMirror schema cannot express: `headingLevels`, `textBlockStyles` and `inlineStyles` with their `appliesTo`, `placeholders`, whether a child block is embedded as `cmsBlock` or `cmsInlineBlock` (`childBlocks`), `maxTextBlocks` and `listLevelMax`.

```json title="block-meta.json (excerpt)"
{
    "name": "tipTapContent",
    ...
    "schema": {
        ...
        "nodes": {
            ...
            "paragraph": {
                "content": "inline*",
                "group": "block",
                "attrs": { "textBlockStyle": { "default": null } }
            },
            "heading": {
                "content": "inline*",
                "group": "block",
                "attrs": { "level": { "default": 2 }, "textBlockStyle": { "default": null } }
            },
            "cmsBlock": {
                "group": "block",
                "atom": true,
...
The `data` attribute of the `link` mark and of `cmsBlock`/`cmsInlineBlock` nodes holds the input of the referenced block, whose fields are described by that block's own entry in the block meta. Content produced from the schema still has to pass the API's regular input validation, so submit it through the usual mutations instead of writing it to the database directly.
