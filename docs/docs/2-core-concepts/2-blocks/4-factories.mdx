...
| Stored data | Untouched                                      | Untouched, lingers untyped                |
...
- **Stored values are not deleted.** Removing a field takes it out of the contract, not out of the database. Values already
  stored under that block name stay in its JSON, untyped, until a migration removes them.
- **Swapping the block at an existing usage site needs no migration on its own.** A `OneOfBlock` resolves by the key it's
  registered under (for instance `external`), not by the block's name, so replacing `external: ExternalLinkBlock` with your
  own block keeps working — the new block just doesn't read fields it doesn't have.
