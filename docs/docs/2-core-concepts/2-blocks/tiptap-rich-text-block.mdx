...
export const TipTapRichTextBlock = createTipTapRichTextBlock({
    link: LinkBlock,
    textBlocks: [
        { name: "paragraph", tag: "p", styles: [{ name: "paragraph300" }, { name: "paragraph200" }] },
...
Content written before a `defaultStyle` was configured carries no style, and the editor fills it in on the first edit rather than when the document is opened, so opening a document does not mark it as changed. To give such content its style up front, run [`buildApplyTextBlocksMigration`](#heading-only-blocks), which also replaces a style a text block no longer offers.
...
export const TipTapRichTextBlock = createTipTapRichTextBlock({
    textBlocks: [
        { name: "paragraph", tag: "p", label: <FormattedMessage id="tipTapRichText.paragraph" defaultMessage="Paragraph" /> },
        { name: "display", tag: "h1", label: <FormattedMessage id="tipTapRichText.display" defaultMessage="Display" /> },
        { name: "heading-1", tag: "h1", label: <FormattedMessage id="tipTapRichText.heading1" defaultMessage="Heading 1" /> },
        { name: "heading-2", tag: "h2", label: <FormattedMessage id="tipTapRichText.heading2" defaultMessage="Heading 2" /> },
    ],
    defaultTextBlock: "paragraph",
});
```

Several text blocks may share a `tag`, as `display` and `heading-1` do above: both are stored as an `h1`, and the node's `textBlock` attribute names which one it is. That covers a design where a display headline sits above a regular heading 1 without being a style of it. On the site, read `node.attrs.textBlock` to tell them apart:

```tsx title="TipTapRichTextBlock.tsx (Site)"
const nodeMapping: Record<string, TipTapNodeHandler> = {
    heading: ({ node, children }) => <Headline variant={node.attrs?.textBlock}>{children}</Headline>,
};
```

The API fills the `textBlock` attribute in when it reads content that doesn't carry it - resolved by the node's tag - so the site can rely on the attribute without falling back to the tag itself.
...
The API rejects content the configuration doesn't allow: a tag no text block uses, or a `textBlock` attribute that names a text block not configured for the tag the node is stored as. Narrowing `textBlocks` for a block that already holds content therefore requires a [migration](./5-migrations.mdx).

A node written before the `textBlock` attribute existed doesn't carry a name and is resolved by its tag when read, so content from before this option keeps working without a migration. That resolution picks the first text block with a matching tag, which only stays stable as long as no second text block uses the same tag: adding a `display` above an existing `heading-1` turns every stored `h1` into a `display`. `buildApplyTextBlocksMigration` writes the resolved name into the content once. Run it with the configuration that is live today, and the existing content keeps the text block it has. It also replaces a `textBlockStyle` the text block (or the list the node sits in) no longer offers with its [`defaultStyle`](#default-styles), for which it takes the list options too:

```ts title="tip-tap-rich-text.block.ts"
import { buildApplyTextBlocksMigration, createTipTapRichTextBlock, type TipTapTextBlock, typeSafeBlockMigrationPipe } from "@dextinity/cms-api";

const textBlocks: TipTapTextBlock[] = [
    { name: "paragraph", tag: "p", styles: [{ name: "copy100" }], defaultStyle: "copy100" },
    { name: "heading-1", tag: "h1" },
];
const orderedList = { styles: [{ name: "list300" }], defaultStyle: "list300" };

export const TipTapRichTextBlock = createTipTapRichTextBlock(
    { textBlocks, orderedList },
    {
        name: "TipTapRichText",
        migrate: {
            version: 2,
            // highlight-next-line
            migrations: typeSafeBlockMigrationPipe([buildApplyTextBlocksMigration({ toVersion: 2, textBlocks, orderedList })]),
        },
    },
);
```

Once it has run, a `display` text block can be added above `heading-1` without touching the content that is already there. Place it after any migration that changes a node's tag (for instance one that bumps heading levels), since it resolves the tag a node has at the point it runs.
