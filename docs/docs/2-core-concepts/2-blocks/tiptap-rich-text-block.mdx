...
| `rte.standardBlockType`                         | [`defaultHeadingLevel`](#restricting-the-text-block-types)                                           |
...
The text block types are part of `supports`: `paragraph`, `heading`, `ordered-list` and `unordered-list`. Leaving `paragraph` out results in a **heading-only block**. Two further options narrow the headings down:
...
The options are validated when the block is created: `headingLevels` must be a non-empty array of unique integers between 1 and 6, and `defaultHeadingLevel` must be one of them. `supports` must contain at least one text block type (`paragraph` or `heading`), and list support requires `paragraph`, because a list item's content starts with a paragraph.

:::caution Existing content

The API rejects content that the configuration doesn't allow: a heading level outside `headingLevels`, or a paragraph in a block without `paragraph` in `supports`. Narrowing these options for a block that already holds content therefore requires a [migration](./5-migrations.mdx). For content coming from the Draft.js block, the built-in [`migrateFromDraftJs`](#migrating-existing-content) migration already converts Draft.js blocks without a heading level into a heading with `defaultHeadingLevel`.
