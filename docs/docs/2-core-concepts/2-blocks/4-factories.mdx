...
The factory brings its own migration for the preview image the block gained after its first release.
It runs for every block the factory creates, so replacing a block a project already has migrates content that was stored before the preview image existed.

That migration occupies version 1, so own migrations start with 2.
The factory throws when a passed `version` or `toVersion` is 1:
...
Run `migrateBlocks` to write the migrated data back:

```bash
pnpm console migrateBlocks
```

Without it the migration still applies whenever a block is loaded, it just isn't persisted until the content is saved.
