...
| Block name  | Optional, defaults to `ExternalLink`         | Mandatory, must be your own               |
...
match an API block of your own. The API name is mandatory because API blocks are registered globally and one name means one
field set, while admin blocks are not registered at all — which is exactly why the same admin block name can appear more
than once, with different `supports`.
...
`fields`. So `fields: []` alone already means the editor sees only the URL. `supports` has to be a subset of `fields` —
the editor can't set an option the block doesn't have — and the factory throws at creation time if it isn't.
