...
The injected Draft.js → TipTap migration is a [vendor migration](./5-migrations.mdx#vendor-migrations), so it doesn't occupy a version of the block. Your own [block migrations](./5-migrations.mdx) start at **version 1** as for any other block:
...
The Draft.js migration used to be version 1 of the block, with own migrations starting at 2.
Existing content keeps being migrated correctly: the version it was saved with is split into the two counters when it is loaded.
