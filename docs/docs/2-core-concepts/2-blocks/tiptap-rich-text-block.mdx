...
| `rte.blocktypeMap` (custom block types)         | [`textBlocks`](#text-blocks) + the text block type select                |
...
export const TipTapRichTextBlock = createTipTapRichTextBlock({
    ...
    textBlocks: [
        ...
        { name: "paragraph-small", tag: "p" },
    ],
    inlineStyles: [{ name: "highlight" }],
    // highlight-start
    migrateFromDraftJs: {
        // Map the Draft.js `blocktypeMap` entry to a text block.
        textBlockMap: {
            "paragraph-small": { textBlock: "paragraph-small" },
        },
...
`textBlock` accepts the `name` of any configured [text block](#text-blocks), and an unknown name throws. A Draft.js block type that isn't mapped becomes the text block its own type implies: `header-one`–`header-six` keep their heading level, everything else becomes a paragraph.
