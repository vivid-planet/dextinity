...
| `rte.blocktypeMap` (custom block types)         | [`textBlockStyles`](#text-blocks-and-styles) + the styling select                             |
...
It uses the block's enabled features and its `textBlocks`, `textBlockStyles` and `maxTextBlocks` options to build the target schema and validates the result. The conversion is **best effort**: if validation fails, it falls back to a stripped-down plain-text document in production (logging a warning) and throws in development so you can catch problems early.
...
export const TipTapRichTextBlock = createTipTapRichTextBlock({
    ...
    textBlockStyles: [{ name: "paragraph200" }],
...
export const TipTapRichTextBlock = createTipTapRichTextBlock({
    ...
    textBlockStyles: [{ name: "headline450" }],
    migrateFromDraftJs: {
        // highlight-next-line
        textBlockStyleMap: {
...
2. **Styling** — a named style (`textBlockStyles`) applied to the current text block. This decouples semantics ("this is a heading") from appearance ("headline 300"), so editors pick a type _and_ a style independently.
...
- the `styles` it offers and the `defaultStyle` new text blocks of this type get,
- a `label` for the toolbar's select (Admin only).

Several text blocks may share a tag, for instance an eyebrow next to a regular paragraph. Both are stored as a paragraph, but each offers its own set of styles.
...
export const TipTapRichTextBlock = createTipTapRichTextBlock({
    link: LinkBlock,
    textBlocks: [
        { name: "paragraph", tag: "p", styles: ["copy200", "copy100"], defaultStyle: "copy200" },
        { name: "eyebrow", tag: "p", styles: ["eyebrow500"] },
        { name: "heading-1", tag: "h1", styles: ["headline300", "headline400"], defaultStyle: "headline300" },
        { name: "heading-2", tag: "h2", styles: ["headline300", "headline400"], defaultStyle: "headline400" },
    ],
    orderedList: { styles: ["copy200", "copy100"], defaultStyle: "copy200" },
    unorderedList: { styles: ["copy200", "copy100"], defaultStyle: "copy200" },
    textBlockStyles: [
        { name: "copy100" },
        { name: "copy200" },
        { name: "eyebrow500" },
...
export const TipTapRichTextBlock = createTipTapRichTextBlock({
    ...
    textBlocks: [
        ...
        },
        {
            ...
            styles: ["headline300", "headline400"],
