...
Blocks are the primary building patterns of a Dextinity CMS application. In this guide, you will learn how to create your first block, from beginning to end.
...
Note how we use the `RichTextBlock`, a Dextinity block, to create our Headline block. Composing a block out of child blocks and additional fields is no problem!
...
```ts title="headline.block.ts"
import { blockInputToData } from "@dextinity/cms-api";
...
Besides the name, the block carries a description of what it is for.
It is written to `block-meta.json` and appears as a comment above the generated TypeScript interfaces, so that tools working with block data can tell the blocks of an application apart.
...
Note how we are using `createBlocksBlock` once again? The helper is available for both the API and the admin part of your Dextinity application!
...
<details>
    ...
    <div>
        <CodeBlock language="ts">
            {`import { Field, FinalFormInput, FinalFormSelect } from "@dextinity/admin";
import { BlockCategory, BlocksFinalForm, createCompositeBlock, createCompositeBlockField, createRichTextBlock } from "@dextinity/cms-admin";
...
In this guide, we've learned how to create a block in a Dextinity CMS project from start to end. We first defined the structure of the block's data in the API, then created the admin component for users to interact with the block, and finally added the block to the site (frontend).

Now you should be ready to create additional blocks! Maybe even a Dog Ear block?

:::info

We've only covered the basic concepts of Dextinity blocks in this guide. To get a full picture, please review the [blocks](/docs/core-concepts/blocks/) section in our documentation.
