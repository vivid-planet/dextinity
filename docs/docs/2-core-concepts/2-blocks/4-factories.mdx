...
option affects the resulting HTTP redirect — use the `createExternalLinkBlock` factory to leave it out.
...
[`createExternalLinkBlock`](#externallinkblock-admin-only) instead of `ExternalLinkBlock`.
