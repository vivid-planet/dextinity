...
Therefore, migrations that ship with the block live in a chain of their own, declared with `vendorVersion` and `vendorMigrations`.
A block factory adds them to the `migrate` option it receives from the application:
...
A block that is entirely owned by the library declares only the vendor chain.

The vendor chain counts its versions from 1, independently of the block's `version`, and its counter is stored per block instance in `$$vendorVersion`:
...
`vendorVersion` and `vendorMigrations` are meant for libraries that provide blocks, for instance Dextinity itself or a package shared between your projects.
For the blocks of your application, keep using `version` and `migrations` — their counter cannot collide with anything.
...
`legacyVendorVersions` states how many of those versions belong to the vendor chain, so the counter is split up when such a block instance is loaded and no migration runs twice:
...
The block's own migrations start at 1 again once the versions before them have moved out of `$$version`, and `legacyVendorVersions` stays at the number of versions that moved, no matter how many migrations either chain adds later.
