...
A mapped block type keeps its text block: `header-two` still becomes the text block with the `h2` tag, only with the mapped `textBlockStyle` applied. If the Draft.js block type doesn't carry that information — a custom block type such as `headline450` that was rendered as `<h2>` — pass a `{ textBlock, textBlockStyle }` object to name the text block explicitly, so the semantic tag isn't lost:
...
`textBlock` accepts the `name` of any configured [text block](#text-blocks). Both parts are optional: pass only `textBlock` to change the text block without applying a style, or only `textBlockStyle` (equivalent to the plain string form) to keep the text block derived from the Draft.js block type.
