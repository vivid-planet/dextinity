...
| `rte.standardBlockType`                         | [`defaultHeadingLevel`](#restricting-the-text-block-types) (+ `allowParagraph: false`)               |
...
### Restricting the text block types

By default, the text block type select offers a paragraph and all six heading levels. Three options narrow this down:

- `headingLevels` — the selectable heading levels. Defaults to `[1, 2, 3, 4, 5, 6]`.
- `defaultHeadingLevel` — the level of newly created headings. Defaults to the lowest level in `headingLevels`.
- `allowParagraph` — set to `false` to remove the paragraph, leaving a **heading-only block**.

A heading-only block is the TipTap equivalent of the Draft.js pattern of a `RichTextBlock` restricted to `header-*` block types with a `standardBlockType` — for instance the headline part of a heading block:
...
The editor starts with an H3, the select only offers _Heading 2_ – _Heading 4_, and `Mod-Alt-2/3/4` switch between them (levels outside `headingLevels` have no shortcut).

The options are validated when the block is created: `headingLevels` must be a non-empty array of unique integers between 1 and 6, and `defaultHeadingLevel` must be one of them. `allowParagraph: false` requires `heading` in `supports` — otherwise no text block type would be left — and cannot be combined with `ordered-list`/`unordered-list`, because a list item's content starts with a paragraph.

:::caution Existing content

The API rejects content that the configuration doesn't allow: a heading level outside `headingLevels`, or a paragraph in a block with `allowParagraph: false`. Narrowing these options for a block that already holds content therefore requires a [migration](./5-migrations.mdx). For content coming from the Draft.js block, the built-in [`migrateFromDraftJs`](#migrating-existing-content) migration already converts Draft.js blocks without a heading level into a heading with `defaultHeadingLevel`.
